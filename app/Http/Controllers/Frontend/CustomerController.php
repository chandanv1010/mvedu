...
use App\Repositories\ConstructRepository;
...
class CustomerController extends FrontendController
{
    ...
    public function recovery(RecoverCustomerPasswordRequest $request){
        ...
        if (!$customer) {
            return redirect()->route('home.index')->with('error', 'Người dùng chưa đăng nhập.');
        }
        ...
        // Use CustomerService to perform the update so business logic stays consistent
        $updated = $this->customerService->update($customer->id, $request);

        if ($updated) {
            return redirect()->route('customer.profile')->with('success', 'Mật khẩu đã được thay đổi thành công.');
        }

        return redirect()->route('customer.profile')->with('error', 'Có lỗi xảy ra khi cập nhật mật khẩu.');
    }
