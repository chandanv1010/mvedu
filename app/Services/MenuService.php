...
class MenuService extends BaseService
{
    ...
    /**
     * Reset main menu - Xóa menu cũ và tạo menu mới
     * 
     * @param int $menuCatalogueId
     * @param array $newMenus Array of ['name' => '', 'canonical' => '']
     * @param int $languageId
     * @return bool
     */
    public function resetMainMenu(int $menuCatalogueId, array $newMenus, int $languageId = 1){
        DB::beginTransaction();
        try{
            // Lấy danh sách menu ID cần xóa để xóa pivot table
            $menusToDelete = $this->menuRepository->findByCondition([
                ['menu_catalogue_id', '=', $menuCatalogueId]
            ], true);
            
            $menuIds = $menusToDelete->pluck('id')->toArray();
            
            // Xóa pivot table (menu_language) trước
            if (count($menuIds) > 0) {
                DB::table('menu_language')
                    ->whereIn('menu_id', $menuIds)
                    ->delete();
            }
            
            // Xóa tất cả menu cũ
            $deletedCount = $this->menuRepository->forceDeleteByCondition([
                ['menu_catalogue_id', '=', $menuCatalogueId]
            ]);

            // Tạo menu mới
            $order = count($newMenus);
            foreach ($newMenus as $menuData) {
                ...
                if ($menuSave->id > 0) {
                    // Tạo pivot với language
                    $payloadLanguage = [
                        'language_id' => $languageId,
                        'name' => $menuData['name'],
                        'canonical' => $menuData['canonical']
                    ];
                    $this->menuRepository->createPivot($menuSave, $payloadLanguage, 'languages');
                }
            ...
            // Khởi tạo lại nested set để tính toán lft, rgt
            $this->initialize($languageId);
            $this->nestedset();
