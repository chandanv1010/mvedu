...
class WidgetService extends BaseService
{
    ...
    private function getWidgetsQuery(array $keywords): Collection
    {
        ...
        return collect($results)->map(function ($widget) {
            ...
            // Decode model_id if it's a JSON string (handle double-encoded JSON)
            if (is_string($widget->model_id)) {
                $decoded = json_decode($widget->model_id, true);
                // If still a string after decode, decode again (double-encoded)
                if (is_string($decoded)) {
                    $decoded = json_decode($decoded, true);
                }
                $widget->model_id = $decoded !== null ? (is_array($decoded) ? $decoded : [$decoded]) : [$widget->model_id];
            } else {
                $widget->model_id = is_array($widget->model_id) ? $widget->model_id : [$widget->model_id];
            }
            return $widget;
        });
    }
    ...
    private function processRegularWidgets(string $model, Collection $widgets, Collection $paramsMap, int $language): array
    {
        ...
        // Handle widgets without model_id (like Text widgets)
        if (empty($modelIds)) {
            $result = [];
            foreach ($widgets as $widget) {
                // For widgets without model_id, just return the widget itself
                $widget->object = $widget->object ?? collect();
                $result[$widget->keyword] = $widget;
            }
            return $result;
    ...
    {
        // Lấy tên class từ model (có thể là App\Models\PostCatalogue hoặc PostCatalogue)
        $className = class_basename($catalogueModel);
        return Str::snake(str_replace('Catalogue', '_catalogue', $className)) . '_id';
    }
    ...
    private function getObjectsWithRelationships(string $tableName, array $modelIds, string $model, int $language): Collection
    {
        ...
        // Lấy tên class từ model (có thể là App\Models\Post hoặc Post)
        $modelName = class_basename($model);
        $modelSnake = Str::snake($modelName);
        
        $languageTable = $modelSnake . '_language';
        $catalogueTable = $modelSnake . '_catalogues';
        $catalogueLanguageTable = $modelSnake . '_catalogue_language';
        $pivotTable = $modelSnake . '_catalogue_' . $modelSnake;
        $modelIdList = implode(',', array_map('intval', $modelIds));
        ...
        $sql .= "
            FROM {$tableName} o
            LEFT JOIN {$languageTable} ol ON o.id = ol.{$modelSnake}_id AND ol.language_id = ?
            LEFT JOIN {$pivotTable} pc ON o.id = pc.{$modelSnake}_id
            LEFT JOIN {$catalogueTable} c ON pc.{$modelSnake}_catalogue_id = c.id
            LEFT JOIN {$catalogueLanguageTable} cl ON c.id = cl.{$modelSnake}_catalogue_id AND cl.language_id = ?
        ";
    ...
    {
        // Nếu model có namespace (App\Models\PostCatalogue), chỉ lấy phần class name cuối cùng
        $className = class_basename($model);
        // Chuyển đổi từ CamelCase sang snake_case và thêm 's'
    ...
    private function getObjectModel(string $catalogueModel): string
    {
        // Lấy tên class từ model (có thể là App\Models\PostCatalogue hoặc PostCatalogue)
        $className = class_basename($catalogueModel);
        return lcfirst(str_replace('Catalogue', '', $className));
    }
    ...
    private function getCatalogueIdFieldForPivot(string $catalogueModel): string
    {
        // Lấy tên class từ model (có thể là App\Models\PostCatalogue hoặc PostCatalogue)
        $className = class_basename($catalogueModel);
        return Str::snake($className) . '_id';
    }
}