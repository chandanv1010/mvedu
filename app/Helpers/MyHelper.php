...
if (!function_exists('image')) {
    function image($image)
    {
        ...
        // Chỉ chạy trên local dev khi có SERVER_IMAGE_URL trong .env
        // File .env không bao giờ được push lên server → server không bị ảnh hưởng
        $serverUrl = env('SERVER_IMAGE_URL', '');
        if (!empty($serverUrl) && !str_starts_with($image, 'http')) {
            $localPath = public_path(ltrim($image, '/'));
            if (!file_exists($localPath)) {
                return rtrim($serverUrl, '/') . '/' . ltrim($image, '/');
            }
        }
